...
(function() {
  var auth, changeDashboard, createGraph, create_dashboards, dashboard, dashboards, dataPoll, default_period, description, generateDataURL, generateEventsURL, generateGraphiteTargets, getTargetColor, graphScaffold, graphs, init, list_to_namespace_dict, list_to_namespace_dict_g, make_counter_dashboard, make_gauge_dashboard, make_timer_dashboard, metrics, period, refresh, refreshSummary, refreshTimer, scheme, toggleCss, _avg, _formatBase1024KMGTP, _max, _min, _sum,
  ...
  createGraph = function(anchor, metric) {
    ...
    return graph = new graph_provider({
      ...
      onComplete: function(transport) {
        var detail, highlighter, shelving, xAxis, yAxis;
  ...
  */
  ...
  make_timer_dashboard = function(namespace, timers) {
    var metric, part, parts, these_metrics, this_dashboard, timer, _i, _len;
    this_dashboard = {
    ...
    for (_i = 0, _len = timers.length; _i < _len; _i++) {
      ...
      metric.targets = (function() {
        var _j, _len1, _ref, _results;
        _ref = ["lower", "mean", "upper_90"];
        _results = [];
        for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
          part = _ref[_j];
          _results.push("alias(" + namespace + "." + timer + "." + part + ", '" + part + "')");
        }
        return _results;
      })();
  ...
  make_counter_dashboard = function(namespace, counters) {
    var counter, metric, these_metrics, this_dashboard, _i, _len;
    this_dashboard = {
  ...
  make_gauge_dashboard = function(namespace, gauges) {
    var gauge, metric, these_metrics, this_dashboard, _i, _len;
    this_dashboard = {
  ...
  list_to_namespace_dict = function(items) {
    var item, key, namespace_dict, value, _i, _len, _ref;
    namespace_dict = {};
    for (_i = 0, _len = items.length; _i < _len; _i++) {
      item = items[_i];
      key = item[0];
      value = item[1];
      if ((_ref = namespace_dict[key]) == null) {
        namespace_dict[key] = new Array();
      }
      if (__indexOf.call(namespace_dict[key], value) < 0) {
  ...
  list_to_namespace_dict_g = function(items) {
    var item, key, namespace_dict, value, _i, _len, _ref;
    namespace_dict = {};
    for (_i = 0, _len = items.length; _i < _len; _i++) {
      item = items[_i];
  ...
  create_dashboards = function() {
    ...
    return deferred.done(function(result) {
      var counters, counters_dict, gauges, gauges_dict, item, k, this_dashboard, timers, timers_dict, v;
      timers = (function() {
        var _i, _len, _results;
        _results = [];
        for (_i = 0, _len = result.length; _i < _len; _i++) {
          item = result[_i];
          if (/stats\.timers/.test(item)) {
            _results.push(item.rsplit(".", 2).slice(0, -1));
          }
        }
        return _results;
      })();
      counters = (function() {
        ...
        for (_i = 0, _len = result.length; _i < _len; _i++) {
          item = result[_i];
          if (/stats\.(?!(gauges|statsd|timers))/.test(item)) {
            _results.push(item.rsplit(".", 1).slice(0));
          }
        }
        return _results;
      })();
      gauges = (function() {
        ...
        for (_i = 0, _len = result.length; _i < _len; _i++) {
          item = result[_i];
          if (/stats\.gauges/.test(item)) {
            _results.push(item.rsplit(".", 2).slice(0));
          }
        }
        return _results;
      })();
      ...
      for (k in timers_dict) {
        if (!__hasProp.call(timers_dict, k)) continue;
        v = timers_dict[k];
        this_dashboard = make_timer_dashboard(k, v);
        dashboards.push(this_dashboard);
      }
      for (k in counters_dict) {
        if (!__hasProp.call(counters_dict, k)) continue;
        v = counters_dict[k];
        this_dashboard = make_counter_dashboard(k, v);
        dashboards.push(this_dashboard);
      }
      for (k in gauges_dict) {
        if (!__hasProp.call(gauges_dict, k)) continue;
        v = gauges_dict[k];
